...
class ScatterEthereumWallet {
    ...
    async getAccounts(callback) {
        ...
        const accounts = !result ? [] : result.accounts
            .filter(account => account.blockchain === Blockchains.ASCH)
...
export default class ASCH extends Plugin {

    constructor(){ super(Blockchains.ASCH, PluginTypes.BLOCKCHAIN_SUPPORT) }
    ...
    async getEndorsedNetwork(){
        return new Promise((resolve, reject) => {
            resolve(new Network('ASCH Mainnet', 
            'https', 
            'mainnet.asch.cn', 
            4096, 
            Blockchains.ASCH, 
    ...
    privateToPublic(privateKey){ 
        const keypair = AschJS.crypto.getKeys(privateKey);
    ...
    randomPrivateKey(){
        return new Promise((resolve, reject) => {
            ...
            // const keypair = AschJS.crypto.getKeys(mnemonic);
